...
2. **Flash MimiBoot:** Copy `mimiboot.uf2` to Pico in BOOTSEL mode

3. **Prepare SD card:**
   - Format as FAT32
   - Create `/boot/kernel.elf` (your payload)
   - Optionally create `/boot.cfg` for configuration

4. **Boot:** Insert SD card, reset Pico — payload runs

For the example payload, build outputs are at `build/boot/kernel.elf`.
...
**Alpha v0.0.1.** Core implementation complete for RP2040. Ready for testing.

- [x] Architecture design
- [x] Handoff protocol specification  
- [x] ELF format requirements
- [x] Core ELF loader (pure C, no dependencies)
- [x] RP2040/RP2350 HAL
- [x] SD card driver (SPI mode)
- [x] FAT32 filesystem (read-only)
- [x] Boot configuration parser
- [x] Example payload (LED blink)
- [x] CMake build system (Pico SDK integration)
- [ ] STM32F4 HAL
